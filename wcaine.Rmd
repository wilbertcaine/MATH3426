...
for (i in 1:mrep) {
  ...
  s_square_phat_est2.5 <- numeric()
  for (j in 1:6) {
    phat_est2.5[[j]] <- mean(sample(data$EST2[data$country == country[j]], n_country[[j]]) == 5)
    s_square_phat_est2.5[[j]] <- phat_est2.5[[j]] * (1 - phat_est2.5[[j]])
  }
  pst_est2.5 <- sum(N_country * phat_est2.5) / sum(N_country)
  var_pst_est2.5 <-
...
```{r, echo = TRUE, warning=FALSE}
sample_size_lower_est2 <- sum(N_country*sqrt(s_square_phat_est2.5))^2/((sum(N_country)*0.02/qnorm(1 - alpha / 2))^2 +sum(N_country*s_square_phat_est2.5))
...
sample_size_lower_est2 <- sum(N_country*s_square_phat_est2.5)/(sum(N_country)*(0.02/qnorm(1-alpha/2))^2 +sum(N_country*s_square_phat_est2.5)/sum(N_country))
sample_size_lower_est2
```
...
```{r, echo = TRUE, warning=FALSE}
n_country_lower_est2 <- round(ceiling(sample_size_lower_est2)*N_country/sum(N_country))
phat_est2.5 <- numeric()
s_square_phat_est2.5 <- numeric()
for (i in 1:6){
  phat_est2.5[[i]] <- mean(sample(data$EST2[data$country==country[i]], n_country_lower_est2[i])==5)
  s_square_phat_est2.5[[i]] <- phat_est2.5[[i]]*(1-phat_est2.5[[i]])
}
pst_est2.5 <- sum(N_country*phat_est2.5)/sum(N_country)
var_pst_est2.5 <- sum((N_country^2)*(1-n_country_lower_est2/N_country)*s_square_phat_est2.5/(n_country_lower_est2-1))/(sum(N_country)^2)
err_bound_pst_est2.5 <- qnorm(1-alpha/2)*sqrt(var_pst_est2.5)
print(list("error bound for random sample with the sample size lower bound"= err_bound_pst_est2.5))
...
for (i in 1:mrep){
  ...
  s_square_phat_est2.5 <- numeric()
  for (j in 1:6) {
    phat_est2.5[[j]] <- mean(sample(data$EST2[data$country == country[j]], n_country_lower_est2[j]) ==  5)
    s_square_phat_est2.5[[j]] <- phat_est2.5[[j]] * (1 - phat_est2.5[[j]])
  }
  pst_est2.5 <- sum(N_country * phat_est2.5) / sum(N_country)
  var_pst_est2.5 <- sum((N_country ^ 2) * (1 - n_country_lower_est2 / N_country) * s_square_phat_est2.5 / (n_country_lower_est2 - 1) ) / (sum(N_country) ^ 2)
...
print(list("relative frequency that the error bound is smaller than 0.02"= mean(temp_p_est2)))
```
...
print(list("point estimate"= ybar_syhs,
...
print(list("point estimate"= ybar_sysd,
           "error bound"= error_bound,
...
print(list("point estimate"= ybar_syrs,
           "error bound"= error_bound,
...
```{r, echo = TRUE, eval=TRUE}
print(list("CI.lower for mu"= ybar_syrs-error_bound,
...
CI_cal <- function(data_mat, ns, n, N){
  ...
  varhat_ybar_syrs <- (1-n/N)*s.2_ybar/ns
  error_bound = qnorm(1-alpha/2)*sqrt(varhat_ybar_syrs)
  list("CI.lower"= ybar_syrs-error_bound, 
...
print(list("relative frequency of those intervals containing the population mean"= mean(temp)))
```
