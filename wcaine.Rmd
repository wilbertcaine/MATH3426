...
if (sum(temp_n_country_lower_est1)<ceiling(sample_size_lower_est1)){
  diff_n <- ceiling(sample_size_lower_est1)-sum(temp_n_country_lower_est1)
  max_index <- numerical(diff_n)
  for (i in 1:diff_n){
    max_index[i] <- 0
    found <- FALSE
    for (j in 1:6){
      ...
      if (!found){
        ...
        else if (temp_n_country_lower_est1[j]>temp_n_country_lower_est1[max_index[i]]){
          max_index[i] = j
        }
      }
    }
    temp_n_country_lower_est1[max_index[i]] <- temp_n_country_lower_est1[max_index[i]]+1
  }
}
...
if (sum(temp_n_country)<5000){
  diff_n <- 5000-sum(temp_n_country)
  max_index <- numerical(diff_n)
  for (i in 1:diff_n){
    max_index[i] <- 0
    found <- FALSE
    for (j in 1:6){
      ...
      if (!found){
        ...
        else if (temp_n_country[j]>temp_n_country[max_index[i]]){
          max_index[i] = j
        }
      }
    }
    temp_n_country[max_index[i]] <- temp_n_country[max_index[i]]+1
  }
}
