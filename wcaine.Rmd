...
print(
  list(
    "relative frequencies for population mean" = mean(temp_mu_ext1),
    "relative frequencies for population total" = mean(temp_tau_ext1)
  )
)
```
...
print(
  list(
    "error bound for random sample" = errorbound_muhat_ext1
  )
)
...
print(
  list(
    "relative frequencies for population proportion" = mean(temp_p_ext2.5)
  )
)
```
...
sample_size_lower_ext2.5 <-
  (N * phat_ext2.5 * (1 - phat_ext2.5)) / (N * V_ext2 + phat_ext2.5 * (1 -
                                                                         phat_ext2.5))
ceiling(sample_size_lower_ext2.5)
```
...
print(
  list(
    "estimate of population mean" = yst_est1,
...
print(
  list(
    "relative frequencies for population mean" = mean(temp_mu_est1)
  )
)
```
...
sample_size_lower_est1 <-
  (sum((N_country ^ 2) * s_square_est1 / w_country)) / ((sum(N_country) *
                                                           0.02 / qnorm(1 - alpha / 2)) ^ 2 + sum(N_country * s_square_est1))
ceiling(sample_size_lower_est1)
```
...
```{r, echo = TRUE, warning=FALSE}
temp_n_country_lower_est1 <-
  ceiling(sample_size_lower_est1) * w_country
if (sum(floor(temp_n_country_lower_est1)) < ceiling(sample_size_lower_est1)) {
  diff_n <-
    ceiling(sample_size_lower_est1) - sum(floor(temp_n_country_lower_est1))
  max_index <- c(0, 0, 0, 0, 0, 0)
  for (i in 1:diff_n) {
    max <- -1
    max_i <- -1
    for (j in 1:6) {
      if ((!max_index[j]) &&
          (max < (
            temp_n_country_lower_est1[j] - floor(temp_n_country_lower_est1[j])
          ))) {
        max <-
          temp_n_country_lower_est1[j] - floor(
            temp_n_country_lower_est1[j])
            max_i <- j
      }
    }
    max_index[max_i] <- 1
    temp_n_country_lower_est1[max_i] <-
      temp_n_country_lower_est1[max_i] + 1
  }
}
n_country_lower_est1 <- floor(temp_n_country_lower_est1)
sample_est1 <- list()
for (i in 1:6) {
  sample_est1[[i]] <-
...
```{r echo=TRUE, warning=FALSE}
alpha <- 0.05
phat_country <- numeric(6)
for (i in 1:6) {
  phat_country[i] <- mean(data$EST2[data$country == country[i]] == 5)
}
s_square_country <- phat_country * (1 - phat_country)
w_country <-
  N_country * sqrt(s_square_country) / sum(N_country * sqrt(s_square_country))
temp_n_country <- 5000 * w_country
if (sum(floor(temp_n_country)) < 5000) {
  diff_n <- 5000 - sum(floor(temp_n_country))
  max_index <- c(0, 0, 0, 0, 0, 0)
  for (i in 1:diff_n) {
    max <- -1
    max_i <- -1
    for (j in 1:6) {
      if ((!max_index[j]) &&
          (max < (temp_n_country[j] - floor(temp_n_country[j])))) {
        max <- temp_n_country[j] - floor(temp_n_country[j])
        max_i <- j
      }
    }
    max_index[max_i] <- 1
    temp_n_country[max_i] <- temp_n_country[max_i] + 1
  }
}
n_country <- floor(temp_n_country)
phat_est2.5 <- numeric(6)
for (i in 1:6) {
  phat_est2.5[i] <-
    mean(sample(data$EST2[data$country == country[i]], n_country[i]) == 5)
}
s_square_phat_est2.5 <- phat_est2.5 * (1 - phat_est2.5)
...
for (i in 1:mrep) {
  ...
  s_square_phat_est2.5 <-
    phat_est2.5 * (1 - phat_est2.5)
  pst_est2.5 <- sum(N_country * phat_est2.5) / sum(N_country)
  var_pst_est2.5 <-
    sum((N_country ^ 2) * (1 - n_country / N_country) * s_square_phat_est2.5 /
          (n_country - 1)) / ((sum(N_country)) ^ 2)
  ...
  temp_phat_est2.5[i] <-
    (CI.lower <= p_est2.5) * (CI.upper >= p_est2.5)
}

print(
  list(
    "relative frequencies for population proportion" = mean(temp_phat_est2.5)
  )
)
```
...
sample_size_lower_est2 <-
  sum(N_country * sqrt(s_square_phat_est2.5)) ^ 2 / ((sum(N_country) * 0.02 /
                                                        qnorm(1 - alpha / 2)) ^ 2 + sum(N_country * s_square_phat_est2.5))
ceiling(sample_size_lower_est2)
```
...
```{r, echo = TRUE, warning=FALSE}
temp_n_country_lower_est2 <-
  ceiling(sample_size_lower_est2) * N_country / sum(N_country)
if (sum(floor(temp_n_country_lower_est2)) < ceiling(sample_size_lower_est2)) {
  diff_n <-
    ceiling(sample_size_lower_est2) - sum(floor(temp_n_country_lower_est2))
  max_index <- c(0, 0, 0, 0, 0, 0)
  for (i in 1:diff_n) {
    max <- -1
    max_i <- -1
    for (j in 1:6) {
      if ((!max_index[j]) &&
          (max < (
            temp_n_country_lower_est2[j] - floor(temp_n_country_lower_est2[j])
          ))) {
        max <-
          temp_n_country_lower_est2[j] - floor(temp_n_country_lower_est2[j])
        max_i <- j
    ...
    max_index[max_i] <- 1
    temp_n_country_lower_est2[max_i] <-
      temp_n_country_lower_est2[max_i] + 1
  }
}
n_country_lower_est2 <- floor(temp_n_country_lower_est2)
phat_est2.5 <- numeric(6)
for (i in 1:6) {
  phat_est2.5[i] <-
...
for (i in 1:mrep) {
  ...
  pst_est2.5 <- sum(N_country * phat_est2.5) / sum(N_country)
  var_pst_est2.5 <-
    sum((N_country ^ 2) * (1 - n_country_lower_est2 / N_country) * s_square_phat_est2.5 / (n_country_lower_est2 - 1)
    ) / (sum(N_country) ^ 2)
  err_bound_pst_est2.5 <-
    qnorm(1 - alpha / 2) * sqrt(var_pst_est2.5)
...
print(list(
  "relative frequency for population mean" = mean(temp)
))
```