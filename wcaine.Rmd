...
print(list("muhat"= ybar_ext1, "tauhat"= tauhat_ext1,
           "error bound for muhat"= errorbound_muhat_ext1, "error bound for tauhat"= errorbound_tauhat_ext1, 
...
CI_cal_mu_ext1 <- function(sample, n, N, alpha){
...
CI_cal_tau_ext1 <- function(sample, n, N, alpha){
  ybar_ext1 <- mean(sample)
  varhat_ybar_ext1 <- (var(sample)/n)*(1-n/N)
...
for (i in 1:mrep){
  sample_ext1 <- sample(data$EXT1, n)
  CI_mu <- CI_cal_mu_ext1(sample_ext1,n,N,alpha)
  temp_mu_ext1[i] <- (CI_mu[[1]]<=mu_ext1)*(CI_mu[[2]]>=mu_ext1)
  CI_tau <- CI_cal_tau_ext1(sample_ext1,n,N,alpha)
...
for (i in 1:mrep){
  sample_ext1 <- sample(data$EXT1, n)
  CI_mu <- CI_cal_mu_ext1(sample_ext1,n,N,alpha)
  temp_mu_ext1[i] <- CI_mu[[3]]<0.015
}

print(list("error bound for random sample with the sample size lower bound from step 4"= errorbound_muhat_ext1,
           "relative frequency that the error bound is smaller than 0.015"= mean(temp_tau_ext1)))
```
